use actix_web::{web, HttpResponse, Resource, error};
use serde_json::json;
use crate::AppState;
use blockchain::block::{NonceNumber};
use blockchain::{ConsensusOption, Blockchain, transaction};
...
async fn handle_post(app_state: web::Data<AppState>) -> Result<HttpResponse, error::Error> {
    match app_state.blockchain.lock() {
        Ok(mut blockchain) => {
            ...
            blockchain.create_new_transaction(transaction::Transaction::new(10, "da Miner".to_owned(), "da Chain".to_owned()));
